...
module SendWithUs
  ...
  class ApiRequest
    ...
    # * *Args*    :
    #   - +configuration+ -> A SendWithUs::Config instance that contains +api_version+
    #   - +end_point+ -> The endpoint for the path
    def self.request_path(configuration, end_point)
    ...
    # A convenience method to call the class method of the same name, with configuration
    def request_path(end_point)
      ApiRequest.request_path(@configuration, end_point)
    end
